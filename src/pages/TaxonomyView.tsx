    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          {skill.isEmerging && (
            <span className="flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-accent/10 text-accent border border-accent/20">
              Emergente
            </span>
          )}
        </div>
        <p className="text-xs text-foreground/75 leading-relaxed">{skill.description}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="border bg-secondary/30 p-2.5">
          ...
          <span className={`inline-block px-2 py-0.5 text-[11px] font-semibold ${TYPE_BADGE[skill.skillType]}`}>
            {TYPE_LABELS[skill.skillType]}
          </span>
        </div>
        <div className="border bg-secondary/30 p-2.5">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Reutilización</p>
          <p className="text-xs font-semibold text-navy">{REUSABILITY_LABELS[skill.reusabilityLevel]}</p>
        </div>
        <div className="border bg-secondary/30 p-2.5">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Fuente</p>
          <p className="text-xs font-semibold text-navy">{skill.source}</p>
        </div>
        <div className="border bg-secondary/30 p-2.5">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Cluster</p>
      ...
      {skill.subcategory && (
        <div className="border bg-secondary/30 p-2.5">
          ...
          <span className={`inline-block px-2 py-0.5 text-[11px] font-semibold ${
            ...
            skill.subcategory === 'Especializada' ? 'bg-accent/10 text-accent border border-accent/20' :
            skill.subcategory === 'Transformacional' ? 'bg-warning/10 text-warning border border-warning/20' :
          ...
          <div className="space-y-1">
            {skill.aliases.map((a, i) => (
              <div key={i} className="flex items-center justify-between border bg-secondary/20 px-2.5 py-1.5">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-muted-foreground font-medium">{SOURCE_LABELS[a.source] ?? a.source}</span>
                  <div className="w-12 h-1 bg-muted overflow-hidden">
                    <div className="h-full bg-primary" style={{ width: `${a.confidence * 100}%` }} />
                  </div>
                  <span className="text-[11px] font-semibold text-navy tabular-nums w-8 text-right">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        <Card className="shadow-metric hover:shadow-metric-hover transition-shadow duration-300">
          <CardContent className="p-2.5 sm:pt-4 sm:pb-4">
            ...
            <p className="text-[10px] sm:text-[11px] text-muted-foreground font-semibold uppercase tracking-wider">Total Skills</p>
          </CardContent>
        </Card>
        <Card className="shadow-metric hover:shadow-metric-hover transition-shadow duration-300">
          <CardContent className="p-2.5 sm:pt-4 sm:pb-4">
            <p className="text-xl sm:text-2xl font-extrabold text-navy tabular-nums">{typeStats.knowledge}</p>
            <p className="text-[10px] sm:text-[11px] text-muted-foreground font-semibold uppercase tracking-wider">Conocimiento</p>
          </CardContent>
        </Card>
        <Card className="shadow-metric hover:shadow-metric-hover transition-shadow duration-300">
          <CardContent className="p-2.5 sm:pt-4 sm:pb-4">
            <p className="text-xl sm:text-2xl font-extrabold text-navy tabular-nums">{typeStats.competence}</p>
            <p className="text-[10px] sm:text-[11px] text-muted-foreground font-semibold uppercase tracking-wider">Competencias</p>
          </CardContent>
        </Card>
        <Card className="shadow-metric hover:shadow-metric-hover transition-shadow duration-300">
          <CardContent className="p-2.5 sm:pt-4 sm:pb-4">
            <p className="text-xl sm:text-2xl font-extrabold text-navy tabular-nums">{typeStats.transversal}</p>
            <p className="text-[10px] sm:text-[11px] text-muted-foreground font-semibold uppercase tracking-wider">Transversales</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="p-3 sm:pt-4 sm:pb-3">
          <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4 flex-wrap">
            <div className="flex-1 min-w-0 sm:min-w-[200px]">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1 block">Buscar</label>
            ...
            <div className="flex gap-2 sm:gap-4">
              ...
              <div className="flex-1 sm:w-36 sm:flex-none">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1 block">Fuente</label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <div className="lg:col-span-2 space-y-1">
          {grouped.map(([category, catSkills]) => {
            ...
            return (
              <div key={category} className="border bg-card shadow-card hover:shadow-card-hover transition-shadow duration-300">
                <button
                  onClick={() => toggleCategory(category)}
                  className="w-full flex items-center justify-between px-3 py-2.5 hover:bg-secondary/30 transition-colors duration-200"
                >
                  <div className="flex items-center gap-2">
                    {isExpanded ? <ChevronDown className="h-3 w-3 text-muted-foreground" /> : <ChevronRight className="h-3 w-3 text-muted-foreground" />}
                    <span className="text-xs font-bold text-navy">{category}</span>
                  </div>
                  <span className="text-[11px] text-muted-foreground font-semibold tabular-nums">{catSkills.length}</span>
                </button>
                {isExpanded && (
                  <div className="border-t">
                    {catSkills.map((skill) => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 text-left hover:bg-primary/[0.03] transition-colors duration-200 border-b last:border-0 ${
                      ...
                      >
                        ...
                        <span className={`shrink-0 px-1.5 py-0.5 text-[10px] font-bold uppercase ${TYPE_BADGE[skill.skillType]}`}>
        ...
        <div className="lg:col-span-3">
          {selectedSkill ? (
            <Card className="shadow-card hover:shadow-card-hover transition-shadow duration-300 sticky top-20">
